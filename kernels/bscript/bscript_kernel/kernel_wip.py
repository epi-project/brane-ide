    ...
    def do_execute(self, code, silent, store_history=True, user_expressions=None, allow_stdin=False):
        ...
        try:
            ...
            for reply in stream:
                ...
                # If it's stdout, publish normally
                if len(reply.stdout) > 0:
                    self.publish_stream("stdout", reply.stdout)
                # If it's error, publish normally
                if len(reply.stderr) > 0:
                    self.publish_stream("stderr", reply.stderr)
                # If it's debug, publish via the extension
                # if len(reply.debug) > 0:
                #     self.publish_status({ "debug": reply.debug })

                # if reply.close:
                #     file_output = self.try_as_file_output(reply.stdout)
